...
namespace FishClient
{
    public partial class FormOnepound : FormMenuBase
    {
        ...
        public FormOnepound(string name)
        {
            InitializeComponent();
            getname = name;
            cmbCountry.SelectedValueChanged -= cmbCountry_SelectedValueChanged;
        ...
        private void btnbangdan_Click(object sender, EventArgs e)
        {
            if (formbangdan == null)
            {
                formbangdan = new FormImage(0, FishEntity.ImageTypeEnum.SGS);
            }

            formbangdan.SetData(_fish == null ? 0 : _fish.Id, FishEntity.ImageTypeEnum.SGS);
