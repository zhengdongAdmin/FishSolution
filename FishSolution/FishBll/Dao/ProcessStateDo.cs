...
namespace FishBll.Dao
{
    public class ProcessStateDo
    {
        ...
        /// 磅单
        /// </summary>
        /// <param name="codeNumbering></param>
        /// <returns></returns>
        ...
        public bool GetFormReceiptRecord(string Numbering)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.AppendFormat("select count(*) from t_receiptrecord where Numbering='" + Numbering + "'and((select sum(ConfirmTheWeight) from t_cargofeedbacksheet where Numbering='" + Numbering + "')*(select unitprice from t_happening where NumberingOne='" + Numbering + "')-(select Sum(FORMAT(Chargeback,2))from t_theproblemsheet where Numbering='" + Numbering + "'))=(select SUM(RMB) from t_receiptrecord where Numbering='"+Numbering+"')");
            if (MySqlHelper.Exists(strSql.ToString()) == true)
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select DISTINCT a.id,a.createman,a.Numbering,a.code,a.codeContract,a.occurDate,a.EventName,a.SolvTtheProblem,a.Remarks,a.Chargeback from t_theproblemsheet a left join t_review b on a.Numbering=b.Numbering and b.programId = 'FormTheproblemsheet' and state = '审核' where a.Numbering='" + Numbering + "'");
            DataTable dt = MySqlHelper.Query(strSql.ToString()).Tables[0];
            if (dt != null && dt.Rows.Count > 0)
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select DISTINCT a.id,a.Numbering,a.createman,a.code,a.codeContract,a.sponsor,a.acceptor,a.processresult,a.evaluation,a.remarks,a.ConfirmTheWeight from t_cargofeedbacksheet a left join t_review b on a.Numbering=b.Numbering and b.programId = 'FormCargoFeedbackSheet' and state = '审核' where a.Numbering='" + Numbering + "'");
            DataTable dt = MySqlHelper.Query(strSql.ToString()).Tables[0];
            if (dt != null && dt.Rows.Count > 0)
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select DISTINCT a.id,a.createman,a.Numbering,a.code codeOne,a.Serialnumber,a.Dateofmanufacture,a.IntothefactoryDate,a.Carnumber,a.Grossweight,a.Tareweight,a.Competition,a.Goods,a.Owner,a.OwnerId,a.Buyers,a.BuyersId,a.Sellers,a.SellersId,a.Country,a.PName,a.Qualit,a.Quantity,a.Pileangle,a.BillOfLadingid,a.Address,a.Remarks,a.Shipno,a.codeContract  from t_poundsalone a left join t_review b on a.Numbering=b.Numbering and b.programId='FormOnepound' AND state = '审核' where a.Numbering='" + Numbering + "'");
            DataTable dt = MySqlHelper.Query(strSql.ToString()).Tables[0];
            if (dt != null && dt.Rows.Count > 0)
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select DISTINCT a.id,a.Numbering,applyCode,a.createMan,a.code codeOne,applyDate,purchasecode,acountNum,unit,contacts,backDeposit,price,weight,paymentMethod,paymentType,paymentDate,invoiceType,remark,case when state='审核' then '审核' else '' end paymentMethodOther from t_paymentrequisition a LEFT JOIN t_review b on a.Numbering=b.Numbering and b.programId='FormPaymentRequisition' and state='审核'");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select DISTINCT a.id,a.Numbering,a.createMan,codeContract,a.code codeTwo,a.fuKuandate,codePrice,codeYunFei,codeHuiKou,fuKuanDanWei,fuKuanZhangHao,weight weightOne ,price priceOne,fuKuanType,fuKuanMethod,RMB,invoiceType invoiceTypeOne,remark remarkOne,case when state='审核' then '审核' else '' end fuKuanOther from t_ReceiptRecord a left join t_review b on a.Numbering=b.Numbering and b.programId='FormReceiptRecord' and b.state='审核' ");
            strSql.AppendFormat(" where a.Numbering='{0}'", Numbering);
