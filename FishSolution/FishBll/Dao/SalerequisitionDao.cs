...
namespace FishBll.Dao
{
    public class SalerequisitionDao
    {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select id,code,supplier,SupplierContact,SupplierContactId,DemandContact,DemandContactId,DemandSideBank,PaymentUnit,RequiredAccount,Purchasingunits,PurchasingunitsId,Purchasingcontacts,PurchasingcontactsId,Purchasecontractnumber,supplierId,demand,demandId,Signdate,Signplace,deIndex,rebate,rebateBool,Portprice,PortpriceBool,Country,CountryBool,transport,deadline,Freight,delivery,dt,dty,lxt,lxty,Settlementmethod,payment,Bank,Receipt,accountnumber,remart,dt,dty,lxt,lxty,WithSkin,WithSkinbool,Numbering,Supplierbool,Demandbool,dbbool,tvnbool,zabool,ffabool,zfbool,sfbool,shybool,szbool,cdbbool,tvnOnebool,hfbool,zaOnebool,ffaOnebool,zfOnebool,sfOnebool,shyOnebool,szOnebool,informationbool,Stockpilebool,Freightbool,rebateSumBool,WithSkinSumbool,RebateSumBool,PortpriceSumBool,FreightSumbool,createman,modifyman from t_salesorder ");
            if (strWhere.Trim() != "")
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select code,Numbering from t_salesorder ");
            if (Numbering.Trim() != "")
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT MAX(code) code FROM t_salesorder where length(code)=13 ");
        ...
        public bool Add(FishEntity.SalesRequisitionEntity model, List<FishEntity.SalesRequisitionBodyEntity> modeList)
        {
            Hashtable SQLString = new Hashtable();
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into t_salesorder(code,supplier,SupplierContact,SupplierContactId,DemandContact,DemandContactId,DemandSideBank,PaymentUnit,RequiredAccount,Purchasingunits,PurchasingunitsId,Purchasingcontacts,PurchasingcontactsId,Purchasecontractnumber,supplierId,demand,demandId,Signdate,Signplace,deIndex,rebate,rebateBool,Portprice,PortpriceBool,CountryBool,transport,deadline,Freight,delivery,dt,dty,lxt,lxty,Settlementmethod,payment,Bank,Receipt,accountnumber,remart,createtime,createman,modifytime,modifyman,WithSkin,WithSkinbool,Numbering,Supplierbool,Demandbool,dbbool,tvnbool,zabool,ffabool,zfbool,sfbool,shybool,szbool,cdbbool,tvnOnebool,hfbool,zaOnebool,ffaOnebool,zfOnebool,sfOnebool,shyOnebool,szOnebool,informationbool,Stockpilebool,Freightbool,rebateSumBool,WithSkinSumbool,PortpriceSumBool,FreightSumbool )");
        ...
        void add_body(FishEntity.SalesRequisitionBodyEntity list, Hashtable SQLString, StringBuilder strSql)
        {
            ...
            strSql.Append("NumberingOne,code,product_id,productname,Funit,Variety,Quantity,Amonut,unitprice,db,tvn,za,ffa,zf,sf,shy,sz,cdb,tvnOne,hf,cm,zjh,tdh,pp,Country,zaOne,ffaOne,zfOne,sfOne,shyOne,szOne) ");
        ...
        public bool Update(FishEntity.SalesRequisitionEntity model, List<FishEntity.SalesRequisitionBodyEntity> modeList)
        {
            ...
            DataSet ds = MySqlHelper.Query("SELECT product_id FROM t_happening WHERE code='" + model.code + "'");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select a.Numbering,a.createman,product_id,a.code,date_format(Signdate,'%Y%m%d')as Signdate,Purchasingunits,d.shortname as GFshortname,c.shortname as XFshortname,PurchasingunitsId,Purchasingcontacts,PurchasingcontactsId,Purchasecontractnumber,Signplace,supplier,SupplierContact,SupplierContactId,DemandContact,DemandContactId,supplierId,DemandSideBank,PaymentUnit,RequiredAccount,demand,demandId,productname,Funit,Variety,Quantity,unitprice,Amonut,a.Freight,db,tvn,za,ffa,zf,sf,shy,sz,cdb,zaOne,ffaOne,zfOne,sfOne,shyOne,szOne,tvnOne,hf,cm,zjh,tdh,rebate,WithSkin,Portprice,b.Country,pp,transport,date_format(deadline,'%Y%m%d')as deadline,Settlementmethod,date_format(payment,'%Y%m%d')as payment,a.Bank,Receipt,accountnumber from t_salesorder a left join t_happening b on a.code=b.code inner JOIN t_company c on a.demand=c.fullname inner JOIN t_company d on a.supplier=d.fullname ");
            strSql.Append("where " + strWhere);
